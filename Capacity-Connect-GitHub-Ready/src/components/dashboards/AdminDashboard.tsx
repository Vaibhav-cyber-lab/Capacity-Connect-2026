...
export const AdminDashboard: React.FC<{ currentView?: string }> = ({ currentView }) => {
  ...
  const handleAddUserSubmit = async (e: React.FormEvent) => {
    ...
    const createdUser: User = {
      ...
      // Admin-created prototype accounts store only a hash, never plaintext.
      passwordHash: await hashPassword(newUserPassword),
      role: newUserRole,
      status: 'approved',
          ...
          <div className="space-y-8 max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Live platform metrics and account activity.</p>
              </div>
              <button 
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Manage registered accounts, roles, and access records.</p>
              </div>
              <button 
                onClick={() => setIsAddUserModalOpen(true)}
