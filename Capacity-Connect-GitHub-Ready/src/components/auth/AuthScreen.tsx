    ...
    if (isLogin) {
      ...
      if (user && (user.passwordHash === passwordHash || user.password === password)) {
        if (!user.passwordHash) {
          // Migrate legacy demo accounts to hashed passwords.
          const migratedUser = { ...user, passwordHash, password: undefined };
          storage.saveUser(migratedUser);
          login(migratedUser);
        } else {
          login(user);
        }
      ...
      const existingUser = storage.getUserByEmail(email.trim().toLowerCase());
      if (existingUser) {
